...
export const handler = async (event: APIAuthorizerEvent): Promise<APIAuthValidationResult> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      //TODO: Esta url a pelo seguramente hay que cambiarla. Quizás se puede sustituir por el uso de cognito SDK.
      // Get PEMS based on Allowd Apps
      let jwks: any[] = [];
      const jwksRes = await axios.get(`https://cognito-idp.eu-west-3.amazonaws.com/${process.env.USER_POOL_ID}/.well-known/jwks.json`);
      ...
      let userId: string = decodedJwtToken.payload.sub?.toString() ?? '';
